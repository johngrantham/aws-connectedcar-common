...
function OutputServiceMetrics($metric) {
  ...
  OutputItemMetric $metric `
      @(@{Name = "TableName"; Value = "${service}_Appointment_Table_${environment}"},@{Name = "GlobalSecondaryIndexName"; Value = "TimeslotAppointmentIndex"}) `
      $start `
      $end `
      "TimeslotAppointmentIndex"

  OutputItemMetric $metric `
      @(@{Name = "TableName"; Value = "${service}_Appointment_Table_${environment}"},@{Name = "GlobalSecondaryIndexName"; Value = "RegistrationAppointmentIndex"}) `
      $start `
      $end `
      "RegistrationAppointmentIndex"

  OutputItemMetric $metric `
      @(@{Name = "TableName"; Value = "${service}_Registration_Table_${environment}"},@{Name = "GlobalSecondaryIndexName"; Value = "VehicleRegistrationIndex"}) `
      $start `
      $end `
